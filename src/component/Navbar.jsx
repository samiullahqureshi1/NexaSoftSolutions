...
const gccGiants = [
  "BEYOND","GBM","SkillBridge","mediaPro","DIFC","SANAD","exa",
  "ADIB","flydubai","Terra Nexus","EMDAD","ATMOSPHERE",
  "SHARJAH","SEPHORA","Khazna","OHANA",
];
...
const ServicesDropdown = ({ closeDropdown, mobile }) => (
  <div
    className={`${
      mobile
        ? "relative mt-4"
        : "absolute top-full left-0 w-full pt-4"
    } z-50`}
  >
    <div className="bg-[#0a0a0a] border border-white/10 rounded-3xl p-6 lg:p-10 shadow-[0_40px_80px_rgba(0,0,0,0.9)] flex flex-col lg:flex-row gap-10 max-h-[70vh] overflow-y-auto">
          ...
          <div key={colIdx} className="space-y-1">
            {column.map((service) => (
              <Link
                ...
                className="group flex items-center justify-between py-3 border-b border-white/5 hover:border-purple-500/40 transition-all"
              >
                <div className="flex items-center gap-3">
      ...
      <div className="lg:w-[340px]">
        <h3 className="text-lg font-bold text-gray-400 uppercase mb-5">
          5-Star Rated <br />
        ...
        <div className="grid grid-cols-4 border-l border-t border-white/10">
          {gccGiants.map((brand, i) => (
            <div
              key={i}
              className="aspect-square border-r border-b border-white/10 flex items-center justify-center p-2 opacity-40 hover:opacity-100 transition"
            >
              <span className="text-[8px] font-black text-white uppercase text-center">
                {brand}
              </span>
            </div>
          ))}
...
const Navbar = () => {
  ...
  return (
    <header className="fixed top-5 left-1/2 -translate-x-1/2 z-[100] w-[95%] max-w-6xl">
      <nav
        className="relative flex items-center justify-between px-6 py-3 rounded-full bg-black/40 backdrop-blur-2xl border border-white/10 shadow-2xl"
        onMouseLeave={() => setServicesOpen(false)}
      >
        {/* LOGO */}
        <Link to="/" className="flex items-center gap-1 text-white text-lg font-semibold">
          <span className="font-light">NexaSoft</span>
          <span>Solutions</span>
        ...
        <div className="hidden sm:block">
          <button className="bg-purple-600 px-6 py-2.5 rounded-full text-sm text-white hover:bg-purple-500 transition">
            Speak to an expert
          </button>
        </div>
        ...
        {servicesOpen && !menuOpen && (
          <ServicesDropdown closeDropdown={() => setServicesOpen(false)} />
        )}
      </nav>

      {/* MOBILE MENU */}
      {menuOpen && (
        <div className="lg:hidden mt-3 bg-[#0a0a0a] border border-white/10 rounded-2xl p-6 space-y-4">
          <button
          ...
          <Link to="/case-studies" className="block text-gray-300 hover:text-white">
            Our Work
          </Link>
          <Link to="/about-us" className="block text-gray-300 hover:text-white">
            About
          </Link>

          <button className="w-full bg-purple-600 py-3 rounded-full text-white">
            Speak to an expert
          </button>
        </div>
      )}
